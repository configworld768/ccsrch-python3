...
class CreditCardScanner:
    def __init__(self, root_path, ignore_list=None, exclude_exts=None, mask=False, max_depth=3, threads=4):
        ...
        self.large_file_threshold = 50 * 1024 * 1024  # 50MB 阈值
    ...
    def process_content(self, text, file_path, archive_chain=None, line_prefix=""):
        """核心扫描逻辑"""
        if not text: return
    ...
    def scan_chunked(self, file_path, archive_chain=None):
        ...
        overlap_size = 256  # 缓冲区重叠，防止卡号被切断
        ...
        try:
            with open(file_path, 'rb') as f:
                ...
                while True:
                    ...
                    # 二进制转文本，忽略非法字符（模拟 strings 命令）
                    current_text = chunk_bytes.decode('utf-8', errors='ignore')
                    
                    # 拼接上一块的尾部，处理边界问题
                    search_text = prev_chunk_tail + current_text
                    
                    # 复用 process_content，标记为 Binary 模式
                    # 注意：这里传入所有文本作为"一行"或者分行处理均可，
                    # 但对于二进制文件，splitlines 可能会产生大量无意义短行。
                    # 这里我们简单地直接调用处理逻辑。
                    self.process_content(search_text, file_path, archive_chain, line_prefix="Bin/Approx-")

                    # 保留尾部供下一块使用
                    prev_chunk_tail = current_text[-overlap_size:] if len(current_text) > overlap_size else current_text

        except Exception as e:
            logger.error(f"Error chunk-scanning {file_path}: {e}")
    ...
    def process_single_file(self, file_path, current_depth=0, archive_chain=None):
        ...
        is_binary_db = ext in ['.ibd', '.frm', '.myd', '.myi', '.db', '.sqlite']
        
        if is_binary_db or file_size > self.large_file_threshold:
            self.scan_chunked(file_path, archive_chain)
        else:
            # 小文本文件正常处理
